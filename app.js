...
// --- 3. ALARME SIRENE 5 SECONDES ---
function playEmergencyAlert(seconds) {
    if (alarmSound) {
        alarmSound.currentTime = 0;
        alarmSound.volume = 1.0;
        alarmSound.play().catch(() => { console.log("Interaction requise pour le son"); });
...
function unlockApp() {
    ...
    playEmergencyAlert(5); // Déclenchement sirène 5 secondes
    ...
    const interval = setInterval(() => {
        ...
        if (timeLeft <= 0) {
            ...
            playEmergencyAlert(2); // Rappel sonore au re-verrouillage
            updateUI();
        }
    }, 1000);
}
